#ifndef UNTITLED_MENUSTATIC_H
#define UNTITLED_MENUSTATIC_H
#include <iostream>
#include <string>
#include "struct.h"
...
void list_lessons(){
    ...
    for(auto& weekday: vec){
        for(auto& item: weekday){
            if(item.auditorium_number != 0) {
                std::cout << line << i << "| ";
                std::cout << weekdays[item.weekday] << " Lesson:" << item.lesson_number << " Auditorium:"
                          << item.auditorium_number << std::endl;
...
bool check_free_auditorias(int day, int les_num, int aud) {
    for (auto& item : vec[day-1]) {
        if (item.lesson_number == les_num && item.auditorium_number == aud) {
            return false;
        }
    }
    return true;
